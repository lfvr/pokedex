...
class PokemonListViewController: UITableViewController, UISearchBarDelegate {
    ...
    var matchedPokemon: [PokemonListResult] = []
    var searching = false
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if searching {
            cell.textLabel?.text = capitalize(text: matchedPokemon[indexPath.row].name)
        } else {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "ShowPokemonSegue",
            ...
            if searching {
                destination.url = matchedPokemon[index].url
            } else {
    ...
    func searchBar(_ searchBar: UISearchBar, textDidChange searchText: String) {
        if searchText.isEmpty == false {
            searching = true
            matchedPokemon = pokemon.filter { $0.name.contains(searchText.lowercased()) }
        } else {
            searching = false
        }
